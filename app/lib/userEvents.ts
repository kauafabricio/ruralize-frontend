...
export function eventToAppointment(event: EventListResponse | EventResponse) {
  ...
  return {
    ...
    category,
    shortDescription: firstParagraph(event.description),
...
export function eventToUserCreatedEvent(
  event: EventResponse | EventListResponse,
) {
...
export function readEventsByCreator(creatorId: string | null | undefined) {
  if (typeof window === "undefined" || !creatorId) {
...
export function eventToInput(event: UserCreatedEvent): UserEventInput {
  const startDate = event.raw?.start_date
    ...
    : parseDisplayDateTime(event.date, event.time);
...
function parseDisplayDateTime(date: string, time: string) {
